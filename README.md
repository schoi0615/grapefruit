...
4.  색상 커스터마이징 모드 추가: 사용자가 '카테고리별' 또는 '크기 순 상위 5개' 지출 항목에 대해 차트 색상을 직접 지정할 수 있는 기능을 추가하여 사용자 정의를 강화했습니다.
5.  UI/UX 개선: Float 기반의 레이아웃 조정을 통해 '기록 추가' 섹션을 최상단에 배치하고, '차트 설정'과 '월간 지출 분석' 섹션을 가로로 나란히 배치하여 대시보드의 가독성을 높였습니다.
6.  RESTful API: 프론트엔드(HTML/JS)와 백엔드(Flask)가 /api/records 엔드포인트를 통해 JSON 형태로 데이터를 주고받습니다.
...
| 프론트엔드 | HTML5, CSS3, JavaScript (Vanilla) | 사용자 인터페이스 및 동작 구현, 색상 커스텀 로직 |
